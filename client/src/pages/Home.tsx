    ...
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section id="team" className="team section-pad">
          ...
          <div className="team-grid">{team.map((person, index) => <article className={`person-card tilt-card reveal delay-${Math.min(index + 1, 3)}`} key={person.id}><div className={`portrait ${person.portrait} ${person.id === "D-02" || person.id === "D-03" || person.id === "D-04" ? "portrait-has-image" : ""}`}>{person.id === "D-02" ? <img className="portrait-image" src="/manus-storage/nik_6c75e50f.jpg" alt="Никита" /> : person.id === "D-03" ? <img className="portrait-image" src="/manus-storage/photo_2025-11-23_15-38-27_a7dba5f0.jpg" alt="Матвей" /> : person.id === "D-04" ? <img className="portrait-image" src="/manus-storage/pasted_file_q0HPlN_image_05d50f75.png" alt="Арсений" /> : <><span className="portrait-placeholder mono">PHOTO / PLACEHOLDER</span><span className="portrait-scan"><ScanLine size={16} /></span><span className="portrait-id mono">{person.id}</span></>}</div><div className="person-meta"><div><h3>{person.name}</h3><p>{person.role}</p></div><ArrowUpRight size={19} strokeWidth={1.2} /></div><p className="person-bio">{person.bio}</p><div className="person-tags">{person.skills.map((skill) => <span key={skill}>#{skill}</span>)}</div></article>)}</div>
        </section>
